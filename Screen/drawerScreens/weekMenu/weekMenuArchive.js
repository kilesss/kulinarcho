...
class weekMenuArchive extends React.Component {
    ...
    async componentDidMount() {
        ...
        this.focusListener = navigation.addListener('didFocus', async () => {
            ...
            if (route === null) {
              arrRoute.push('weekMenuArchive')
            } else {
              arrRoute = route
            }
            if (arrRoute[arrRoute - 1] != 'weekMenuArchive') {
              arrRoute.push('weekMenuArchive')
            }       
    ...
    showRecipe(id){
        AsyncStorage.setItem('ArchiveWeekMenuID',id.toString()).then(data => {
            this.props.navigation.navigate('weekMenuArchiveDetailed', {name: 'kuyr'});
...
export default weekMenuArchive;
